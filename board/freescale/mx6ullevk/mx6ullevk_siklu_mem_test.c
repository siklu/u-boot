...
typedef enum {
	NXP_MEM_TEST_FIRST = 1,
	NXP_MEM_TEST_RUNNING_ONE = NXP_MEM_TEST_FIRST,
	NXP_MEM_TEST_RUNNING_ZERO,
	NXP_MEM_TEST_CHESS,
	NXP_MEM_TEST_RANDOM_PTRN,
	NXP_MEM_TEST_ADDR_2_DATA,
	NXP_MEM_TEST_LAST
// should be last
} NXP_MEM_TEST_E;
 ...
 *
 */
static int nxp_memtest_running_one(ulong start_addr, ulong end_addr,
		int is_cycling) //
{
	int rc = 0;
 ...
 *
 */
static int nxp_memtest_running_zero(ulong start_addr, ulong end_addr,
		int is_cycling) //
{
	int rc = 0;
	volatile uint* temp_addr;
 ...
 *
 */
static int nxp_memtest_chess(ulong start_addr, ulong end_addr,
		int is_cycling) {
	int rc = 0;
	volatile uint* temp_addr;
	int pass_num = 1;
 ...
 *
 */
static int nxp_memtest_random(ulong start_addr, ulong end_addr,
		int is_cycling) {
	int rc = 0;
 ...
 *
 */
static int nxp_memtest_addr2data(ulong start_addr, ulong end_addr,
		int is_cycling) {
	int rc = 0;
	volatile uint* temp_addr;
	int pass_num = 1;
		...
		char * const argv[]) //
{
	...
	NXP_MEM_TEST_E test_type = NXP_MEM_TEST_ADDR_2_DATA;
	int is_cyclic = 0;

	// default values
	start_addr = CONFIG_SYS_MEMTEST_START;
	end_addr = 0x2000000 + CONFIG_SYS_MEMTEST_START;
	...
	// check parameters
	if ((test_type < NXP_MEM_TEST_FIRST)
			|| (test_type >= NXP_MEM_TEST_LAST)) {
		printf(" Unknown test type %d\n", test_type);
		return 1;
	}

	if ( (end_addr - start_addr) < 1000000)
	{
		printf("The difference between End address and start address must be greater than 1M\n");
		return 1;
	}

	start_addr += CONFIG_SYS_MEMTEST_START;
	end_addr += CONFIG_SYS_MEMTEST_START;

	printf(" start:0x%2lx End:0x%2lx\n", start_addr, end_addr);
	...
	switch (test_type) {
	case NXP_MEM_TEST_RUNNING_ONE:
		rc = nxp_memtest_running_one(start_addr, end_addr, is_cyclic);
		break;
	case NXP_MEM_TEST_RUNNING_ZERO:
		rc = nxp_memtest_running_zero(start_addr, end_addr, is_cyclic);
		break;
	case NXP_MEM_TEST_CHESS:
		rc = nxp_memtest_chess(start_addr, end_addr, is_cyclic);
		break;
	case NXP_MEM_TEST_RANDOM_PTRN:
		rc = nxp_memtest_random(start_addr, end_addr, is_cyclic);
		break;
	case NXP_MEM_TEST_ADDR_2_DATA:
		rc = nxp_memtest_addr2data(start_addr, end_addr, is_cyclic);
		break;
	default:
		break;

...
// All supported boards should be here
...
siklu_mrv_system_seeprom.seeprom_board_info.header.crc.val = crc32((unsigned int) SYS_EEPROM_CRC_SEED_PRIM_VAL,
        (unsigned char *) &siklu_mrv_system_seeprom.seeprom_board_info.data, (int) sizeof(seeprom_data_S));

// last step - write data to seeprom
seeprom_erase_all_section();  // SPI NOR FLASH should be erased before write
mdelay(100);
rc = spi_flash_write(_get_spi_flash_data(), get_syseeprom_area_snor_offs(), sizeof(seeprom_S),
        &siklu_mrv_system_seeprom);
if (rc != 0)
{
...
if (siklu_mrv_system_seeprom.seeprom_board_info.header.control_key.val != SYS_EEPROM_CONTROL_KEY_VAL)
{
    printf(" Control Key is Wrong. Expected 0x%x, Read 0x%x\n",
    SYS_EEPROM_CONTROL_KEY_VAL, siklu_mrv_system_seeprom.seeprom_board_info.header.control_key.val);
    return -1;
}

// calculate and compare CRC
crc = crc32(SYS_EEPROM_CRC_SEED_PRIM_VAL, (__u8 *) &siklu_mrv_system_seeprom.seeprom_board_info.data,
...
{
    printf("%s() Wrong CRC. Exists 0x%x, Calculated 0x%x, seeprom_raw_data %p, calculated size %d\n", __func__,
            siklu_mrv_system_seeprom.seeprom_board_info.header.crc.val, crc, &siklu_mrv_system_seeprom,
...
int seeprom_primary_format_v1(void)
{
    ...
    siklu_mrv_system_seeprom.seeprom_board_info.data.serial_bb.t = PROTECTED_FLASH_TYPE_BB_SERIAL_NUMBER;
    siklu_mrv_system_seeprom.seeprom_board_info.data.serial_bb.l =
    NVRAM_SERIAL_NUM_FIELD_MAX_SIZE;
    tmp = "233333332";
    memcpy(siklu_mrv_system_seeprom.seeprom_board_info.data.serial_bb.serial, tmp, strlen(tmp));

    // Set System serial
    siklu_mrv_system_seeprom.seeprom_board_info.data.serial_sys.t = PROTECTED_FLASH_TYPE_SYSTEM_SERIAL_NUMBER;
    siklu_mrv_system_seeprom.seeprom_board_info.data.serial_sys.l =
    NVRAM_SERIAL_NUM_FIELD_MAX_SIZE;
    tmp = "";
    memcpy(siklu_mrv_system_seeprom.seeprom_board_info.data.serial_sys.serial, tmp, strlen(tmp));

    // Set Network port types
    siklu_mrv_system_seeprom.seeprom_board_info.data.netw_port.t = PROTECTED_FLASH_TYPE_NETWORK_PORT_TYPE;
    siklu_mrv_system_seeprom.seeprom_board_info.data.netw_port.l =
    NVRAM_NETW_PORT_TYPE_FIELD_SIZE;
    strcpy(siklu_mrv_system_seeprom.seeprom_board_info.data.netw_port.port_type, "ccf-");

    // Set Product Name
    siklu_mrv_system_seeprom.seeprom_board_info.data.product_name.t = PROTECTED_FLASH_TYPE_PRODUCT_NAME;
    siklu_mrv_system_seeprom.seeprom_board_info.data.product_name.l =
    NVRAM_PRODUCT_NAME_FIELD_SIZE;
    strcpy(siklu_mrv_system_seeprom.seeprom_board_info.data.product_name.product, "EH700");

    // Set assembly type
    siklu_mrv_system_seeprom.seeprom_board_info.data.assembly.t = PROTECTED_FLASH_TYPE_ASSEMBLY_TYPE;
    siklu_mrv_system_seeprom.seeprom_board_info.data.assembly.l =
    NVRAM_ASSEMBLY_TYPE_FIELD_SIZE;
    strcpy(siklu_mrv_system_seeprom.seeprom_board_info.data.assembly.type, "FABB031A");

    // after complete  init all TLV set header info
    siklu_mrv_system_seeprom.seeprom_board_info.header.num_tlv.val = 6;
    siklu_mrv_system_seeprom.seeprom_board_info.header.data_size.val = sizeof(seeprom_data_S);
...
int seeprom_get_system_serial_v1(char* val)
{
    ...
    if (rc == 0)
    {
        memcpy(val, siklu_mrv_system_seeprom.seeprom_board_info.data.serial_sys.serial,
        NVRAM_SERIAL_NUM_FIELD_MAX_SIZE);
    }
    else
    {
...
int seeprom_get_baseband_serial_v1(char* val)
{
    ...
    if (rc == 0)
    {
        memcpy(val, siklu_mrv_system_seeprom.seeprom_board_info.data.serial_bb.serial,
        NVRAM_SERIAL_NUM_FIELD_MAX_SIZE);
    }
    else
    {
...
int seeprom_set_baseband_serial_v1(char* val)
{
    ...
    NVRAM_SERIAL_NUM_FIELD_MAX_SIZE);
    memcpy(siklu_mrv_system_seeprom.seeprom_board_info.data.serial_bb.serial, val, strlen(val));
...
int seeprom_get_product_name_v1(char* product_name)
{
    ...
    if (rc == 0)
    {
        memcpy(product_name, siklu_mrv_system_seeprom.seeprom_board_info.data.product_name.product,
        NVRAM_PRODUCT_NAME_FIELD_SIZE);
    }
    else
    {
...
int seeprom_set_product_name_v1(char* product_name)
{
    ...
    siklu_mrv_system_seeprom.seeprom_board_info.data.product_name.l =
    NVRAM_PRODUCT_NAME_FIELD_SIZE;
    memset(siklu_mrv_system_seeprom.seeprom_board_info.data.product_name.product, 0, NVRAM_PRODUCT_NAME_FIELD_SIZE);
    strcpy(siklu_mrv_system_seeprom.seeprom_board_info.data.product_name.product, product_name);
...
int seeprom_set_assembly_type_v1(char* assembly)
{
    ...
    {
        //printf("%s, line %d, i %d, supported_board_assembly[i] %s\n",
        //        __func__, __LINE__, i, supported_board_assembly[i]);
        if (strcmp(supported_board_assembly[i], assembly) == 0)
        {
            rc = 0;
            break;
        }
    ...
    siklu_mrv_system_seeprom.seeprom_board_info.data.assembly.l =
    NVRAM_ASSEMBLY_TYPE_FIELD_SIZE;
    memset(siklu_mrv_system_seeprom.seeprom_board_info.data.assembly.type, 0, NVRAM_ASSEMBLY_TYPE_FIELD_SIZE);
    strcpy(siklu_mrv_system_seeprom.seeprom_board_info.data.assembly.type, assembly);
...
int seeprom_get_assembly_type_v1(char* assembly)
{
    ...
    if (rc == 0)
    {
        memcpy(assembly, siklu_mrv_system_seeprom.seeprom_board_info.data.assembly.type,
        NVRAM_ASSEMBLY_TYPE_FIELD_SIZE);
    }
    else
    {
 ...
 *      '-' - non-existant port
 *
 *      19 oct 2016: set network's port types not allowed. The value defined by board assembly type only!
 */
int seeprom_set_netw_port_map_v1(const char* port_map)
{
    ...
    {
        printf("Network port map should be exact 4 characters lens, received %d\n", strlen(port_map));
        return 0;
    }
    if (!(is_port_type_valid(port_map[0]) && is_port_type_valid(port_map[1]) && is_port_type_valid(port_map[2])
            && is_port_type_valid(port_map[3])))
    {
        printf("Network port map values should be 'c'=copper or 'f'=fiber or '-'=no physical port\n");
        return 0;
    }
    ...
    siklu_mrv_system_seeprom.seeprom_board_info.data.netw_port.l =
    NVRAM_NETW_PORT_TYPE_FIELD_SIZE;
    memcpy(siklu_mrv_system_seeprom.seeprom_board_info.data.netw_port.port_type, port_map,
    NVRAM_NETW_PORT_TYPE_FIELD_SIZE);
...
/*
 * 19 oct 2016: set network's port types not allowed. The value defined by board assembly type only!
 * The function returns network port map based on board assembly type
 */
const char* seeprom_get_netw_port_map_v1(void)
{
    ...
    seeprom_get_assembly_type_v1(assembly);

    for (i=0;i<(sizeof(asm2netw_port_map)/sizeof(asm2netw_port_map_S));i++)
    {
        if(strcmp(asm2netw_port_map[i].assembly,assembly) == 0)
        {
            memcpy(netw_port_map, asm2netw_port_map[i].map, 4);
            break;
        }
    }

...
int siklu_rtc_get(struct rtc_time *val)
{
    ...
    val->tm_sec = bcd2bin(sec & 0x7f);
    val->tm_min = bcd2bin(min & 0x7f);
    val->tm_hour = bcd2bin(hour & 0x3f);
    val->tm_mday = bcd2bin(mday & 0x3f);
    val->tm_wday = wday & 0x7;
    val->tm_mon = bcd2bin(mon & 0x1f);
    val->tm_year = bcd2bin(year) + (bcd2bin(year) >= 70 ? 1900 : 2000);

    DEBUGR("Get DATE: %4d-%02d-%02d (wday=%d)  TIME: %2d:%02d:%02d\n", val->tm_year, val->tm_mon, val->tm_mday,
            val->tm_wday, val->tm_hour, val->tm_min, val->tm_sec);
...
int siklu_rtc_set(struct rtc_time *val)
{
    ...
    if (val->tm_year < 1970 || val->tm_year > 2069)
        printf("WARNING: year should be between 1970 and 2069!\n");

    pcf8523_stop_rtc();
    rtc_write(REG_SECONDS, bin2bcd(val->tm_sec));
    rtc_write(REG_MINUTES, bin2bcd(val->tm_min));
    rtc_write(REG_HOURS, bin2bcd(val->tm_hour));
    rtc_write(REG_DAYS, bin2bcd(val->tm_mday));
    rtc_write(REG_WEEKDAYS, val->tm_wday);
    rtc_write( REG_MONTHS, bin2bcd(val->tm_mon));
    rtc_write( REG_YEARS, bin2bcd(val->tm_year % 100));
